 ...
 * @class Fflib.data.proxy.VFRemote
 * 
 * This component limits our Ext proxy integration to the interface provided here:
 * http://www.salesforce.com/us/developer/docs/pages/Content/pages_js_remoting.htm
 * 
 * This is because we control neither the transport nor the implementation of the remoting client. "Keyholing" our
 * interactions through `Visualforce.remoting.Manager.invokeAction` ensures we don't rely on client properties such
 * as `ns.RemoteController.action.directCfg.method`, and encourages use of the namespace-resolving $RemoteAction global.
 * 
 * Tip: to use this in Sencha Architect, specify proxy.createAlias of 'vfremote'
 */
Ext.define('Fflib.data.proxy.VFRemote', {
    
    /* Begin Definitions */
    
    extend: 'Ext.data.proxy.Direct',
    requires: ['Ext.direct.RemotingMethod'], //We are pinching the implementation of getArgs
    alias: 'proxy.vfremote',
    ...
    doRequest: function(operation, callback, scope) {
        ...
        if (!fn) Ext.Error.raise('[Fflib.data.proxy.VFRemote] No direct function specified for this proxy');
        ...
        if (operation.action == 'read') {
            ...
            if (Ext.Object.getSize(params) != data.length) Ext.Error.raise('[Fflib.data.proxy.VFRemote] paramOrder must account for exactly ' + data.length + ' parameters');
        } else {
            data.push(request.jsonData);
        }
